...
\begin{frame}{Proposition 1 - Key Insight for Computability}
    ...
    \textbf{It means:}
     \begin{beamercolorbox}[rounded=true,shadow=true,sep=0.75em,wd=1\textwidth,center]{block body}
        \small
        ``All solutions can be found if we can find the \textbf{upper bound} of all variables in LHS or RHS ''
    \end{beamercolorbox}
...
\begin{frame}{Modeling the Modulo Trick with Sets}
    \begin{block}{What happens when we apply the modulo trick?}
        \begin{itemize}
            ...
            \item We repeat with $m_2, m_3, \dots$ -- each time further restricting the possibilities
        \end{itemize}
    \end{block}
...
\begin{frame}{Theorem 1 - Non-solvability of $1+(pq)^a = p^b + q^c$}
    ...
    \begin{enumerate}
        \item By Lemma 3, it suffices to show for \textbf{any} modulus $m$, $A_m$ 
        contains arbitrarily large $a,b,c$
        \item Create a form of solution that can generate arbitrarily large solutions.
    \end{enumerate}
...
\begin{frame}{Moving Beyond the Specific Form}
    ...
    \begin{itemize}
        ...
        \item But, We want to analyze other forms like:
        \begin{align*}
...
\begin{frame}{General Form of Exponential Diophantine Equation}
    \begin{definition}[General Form]
        Let $n,m \in \mathbb{Z}^{+}$, and $C_1, C_2 \in \mathbb{Z}_{\geq 0}$ and $k_i, a_i, l_j, b_j \in \mathbb{Z}^{+}$ for $i=1,2,\dots,n$ and $j=1,2,\dots,m$. 
        
        Consider the exponential Diophantine equation
        \begin{equation}
            C_1 + \sum_{i=1}^n k_i a_i^{x_i} = C_2 + \sum_{j=1}^m l_j b_j^{y_j} \label{general}
        \end{equation}
        where $x_1,x_2,\dots,x_n, y_1,y_2,\dots,y_m \in \mathbb{Z}_{\geq 0}$.
    \end{definition}
    
    \textbf{Examples rewritten in this form:}
    \begin{align*}
        1 + 5^a &= 7^b + 7^c \quad \small(C_1=1, n=1, k_1=1, a_1=5, C_2=0, m=2, l_1=l_2=1, b_1=b_2=7)\\
        p^x - p^y &= q^z - q^w \quad (C_1=0, n=2, k_1=1, k_2=-1, a_1=a_2=p, C_2=0, m=2, l_1=1, l_2=-1, b_1=b_2=q)
    \end{align*}
    
    \textbf{Significance:} Almost every form of eDe can be written in this form!
\end{frame}
...
\begin{frame}{Key Lemma - Generalized Single modulus lemma}
    ...
    \textbf{Significance:}
    \begin{itemize}
    \item  Generalization of Lemma 1
        \item Any sequence of modular restrictions reduces to a \textbf{single modulus} (the LCM)
        \item Allows us to prove non-solvability by considering arbitrary single modulus
    \end{itemize}
\end{frame}
\begin{frame}{Lemma 3 - Period of Exponent Residues}
    ...
    \textbf{Key insight:} Exponents can grow while maintaining congruence patterns
...
\begin{frame}{Theorem 2 - Non-solvability of $a^x-a^y = b^z-b^w$}
    ...
    \textbf{Proof Strategy:}
    \begin{enumerate}
        \item  By Generalized Single modulus lemma to reduce the problem.
        \item  Use Lemma 3 to generate solutions that grow without upper bound in any modulo.
    \end{enumerate}
\end{frame}
